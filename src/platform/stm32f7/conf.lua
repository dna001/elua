...
addi( sf( 'src/platform/%s/FWLib/library/inc/Legacy', platform ) )

local fwlib_files = utils.get_files( "src/platform/" .. platform .. "/FWLib/library/src", ".*%.c$" )
specific_files = "system_stm32f7xx.c startup_stm32f769xx.s stm32f7xx_it.c platform.c platform_int.c cpu.c stm32_pio.c"
...
addm( { "FOR" .. cnorm( comp.cpu ), "FOR" .. cnorm( comp.board ), 'gcc', 'STM32F769xx', 'CORTEX_M4' } )
...
tools.stm32f7.pre_build = function()
...
tools.stm32f7.prog_flist = { output .. ".bin", output .. ".hex" }

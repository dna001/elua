// PLL configuration for the STM32F7 platform
...
#define STM32F7_DESIRED_SYSCLK_FREQ_MHZ           ( ELUA_BOARD_CPU_CLOCK_HZ / 1000000 )
#else
#define STM32F7_DESIRED_SYSCLK_FREQ_MHZ           216
#endif

#define STM32F7_CRYSTAL_FREQ_MHZ                  ( ELUA_BOARD_EXTERNAL_CLOCK_HZ / 1000000 )

#define STM32F7_F_OTHER_FREQ_MHZ                  48

#define PLL_M                                     ( STM32F7_CRYSTAL_FREQ_MHZ )
#if PLL_M < 2 || PLL_M > 63
#error PLL_M out of range, unable to compute PLL parameters
#endif

#if STM32F7_DESIRED_SYSCLK_FREQ_MHZ <= 100
#define PLL_P                                     4
#else
#define PLL_P                                     2
#endif
#define PLL_N                                     ( STM32F7_DESIRED_SYSCLK_FREQ_MHZ * PLL_P )
#if PLL_N < 192 || PLL_N > 432
#error PLL_N out of range, unable to compute PLL parameters
#endif

#define F_VCO                                     ( ( STM32F7_CRYSTAL_FREQ_MHZ * PLL_N ) / PLL_M )
#define STM32F7_ACTUAL_SYSCLK_FREQ_MHZ            ( F_VCO / PLL_P )
#define PLL_Q                                     ( F_VCO / STM32F7_F_OTHER_FREQ_MHZ )
#if ( F_VCO % STM32F7_F_OTHER_FREQ_MHZ ) != 0

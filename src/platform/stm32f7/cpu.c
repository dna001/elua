...
{
  lua_newtable( L );
  lua_pushinteger( L, HAL_RCC_GetSysClockFreq() );
  lua_setfield( L , -2, "SYSCLK" );
  lua_pushinteger( L, HAL_RCC_GetHCLKFreq() );
  lua_setfield( L , -2, "HCLK" );
  lua_pushinteger( L, HAL_RCC_GetPCLK1Freq() );
  lua_setfield( L , -2, "PCLK1" );
  lua_pushinteger( L, HAL_RCC_GetPCLK2Freq() );
...
static int cpu_rng_setup( lua_State *L )
{
  ...
  RNG->SR = ~(u32)(( RNG_FLAG_CECS | RNG_FLAG_SECS ) << 4);
  /* Enable the RNG */
  RNG->CR |= RNG_CR_RNGEN;
  rng_initialised = 1;
...
static int cpu_rng_read( lua_State *L )
{
  ...
  while( ( RNG->SR & RNG_FLAG_DRDY ) == 0 ) {
    if( ( ( RNG->SR & RNG_FLAG_CECS ) != 0 ) ||
        ( ( RNG->SR & RNG_FLAG_SECS ) != 0 ) )
    {
      rng_initialised = 0;
      lua_pushnil( L );
      return 1;
